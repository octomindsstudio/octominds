...
title: Looking for a creative frontend developer to build a high-end interactive website.
tools:
  - Motion
  - NextJs
  - Tailwind
...
<PullQuote author="Vercel Ship 2024" role="Community Picks">
  "PrettyShot is the kind of utility that makes you wonder why it wasn't built
  sooner. It's the ultimate tool for developers who care about presentation."
</PullQuote>
...
<Callout type="note" title="Privacy by default">
  Your images never leave your browser. PrettyShot doesn't upload anything to
  any server. The entire rendering and export pipeline runs locally.
</Callout>
...
<FeatureGrid cols={2}>
  <FeatureCard
    ...
    title="12+ Gradient Presets"
    description="Sunset, Ocean, Forest, Midnight, plus 6 mesh backgrounds for more organic looks."
  />
  <FeatureCard
    icon={<Layers3 size={24} />}
    title="Dynamic Shadows"
    description="7 presets from subtle to dramatic, each with a color picker to match any brand."
  />
  <FeatureCard
    icon={<Move3D size={24} />}
    title="3D Perspective"
    description="Full X, Y, and Z rotation sliders for dynamic, high-end hero shots."
  />
  <FeatureCard
    icon={<Copy size={24} />}
    title="Clipboard Integration"
    description="One-click copy to clipboard functionality for seamless workflow integration."
  />
</FeatureGrid>
