...
export async function getBlogs({
  ...
  const allPages = source.getPages().filter((p) => p.slugs.join("/") !== "");
  ...
  const filtered = allPages.filter((entry) => {
    ...
    const inTags = entry.data.tags?.some((t) =>
  ...
  const blogsWithViews = filtered.map((entry) => {
    return {
